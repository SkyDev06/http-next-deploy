...
class AdminCreator:
    ...
    def _recover_prefixes(self, src):
        ...
        Multisite:   wp_1_posts → blog=wp_1_ base=wp_  (users live on base_prefix)
        """
        ptable = src.extract(
        ...
        # Prefer real users table name from schema
        utable = src.extract(
            "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() "
            "AND RIGHT(TABLE_NAME,5)=0x7573657273 ORDER BY CHAR_LENGTH(TABLE_NAME),TABLE_NAME LIMIT 1"
        )  # ends with 'users'
        base_prefix = None
        if utable and re.fullmatch(r"[A-Za-z0-9_$]+", utable) and utable.endswith("users"):
            base_prefix = utable[:-5]

        # Multisite: blog prefix often wp_N_ while base is wp_
